...
CCamera::CCamera()
{
	...
	m_projectMatrix = glm::perspective(glm::radians(FRUSTUM_FOVY), static_cast<float>(WINDOW_WIDTH) / static_cast<float>(WINDOW_HEIGHT)
...
void CCamera::Render()
{
	...
	glUniformMatrix4fv(glGetUniformLocation(program, "projectionTransform"), 1, GL_FALSE, glm::value_ptr(m_projectMatrix));
}
